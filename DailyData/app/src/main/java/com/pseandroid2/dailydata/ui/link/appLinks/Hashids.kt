...
class Hashids(
    ...
    fun decode(hash: String): LongArray = when {
        hash.isEmpty() -> longArrayOf()
            ...
            val returnValue =
                unhashSubHashes(hashBreakdown.iterator(), lottery, mutableListOf(), finalAlphabet)
    ...
    private fun calculateAlphabetAndSeparators(userAlphabet: String): AlphabetAndSeparators {
        ...
        when {
            ...
            uniqueAlphabet.contains(space) -> throw IllegalArgumentException("alphabet cannot contains spaces")
                ...
                val alphabetWithoutSeparators =
                    uniqueAlphabet.toSet().minus(legalSeparators).joinToString(emptyString)
                val shuffledSeparators =
                    consistentShuffle(legalSeparators.joinToString(emptyString), finalSalt)
    ...
    private fun adjustAlphabetAndSeparators(
        ...
        if (shuffledSeparators.isEmpty() ||
            ...
            AlphabetAndSeparators(
                consistentShuffle(alphabetWithoutSeparators, finalSalt),
                shuffledSeparators
            )
        }
    ...
    private tailrec fun shuffle(data: ShuffleData, currentPosition: Int, limit: Int): ShuffleData =
        when {
    ...
    private tailrec fun ensureMinimalLength(
        ...
        returnString.length < finalHashLength -> {
            ...
            val tempReturnString =
                newAlphabet.substring(halfLength) + returnString + newAlphabet.substring(
                    0,
                    halfLength
                )
